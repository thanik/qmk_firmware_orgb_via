...
#    define OPENRGB_DIRECT_MODE_STARTUP_RED 2
#endif

#if !defined(OPENRGB_DIRECT_MODE_STARTUP_GREEN)
#    define OPENRGB_DIRECT_MODE_STARTUP_GREEN 2
#endif

#if !defined(OPENRGB_DIRECT_MODE_STARTUP_BLUE)
#    define OPENRGB_DIRECT_MODE_STARTUP_BLUE 2
#endif
...
void openrgb_get_led_info(uint8_t *data) {
    ...
    for (uint8_t i = 0; i < number_leds; i++) {
        ...
        if (col >= MATRIX_COLS || row >= MATRIX_ROWS) {
            ...
            raw_hid_buffer[data_idx + 7] = keymap_key_to_keycode(0, (keypos_t) {col, row});
        }
    }
}
